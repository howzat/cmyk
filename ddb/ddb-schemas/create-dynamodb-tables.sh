...
for SCHEMA in "${SCHEMA_FOLDER}"/*.json; do
    ...
    table_exists=$(aws dynamodb list-tables --endpoint-url "${DYNAMO_ENDPOINT}" --region ${AWS_REGION-eu-west-2} | jq "(.TableNames | length > 0) and (.TableNames | contains([\"${tablename}\"]))")
